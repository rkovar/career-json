...
def render_review(state):
    ...
    for row in rows:
        ...
        if related:
            ...
            content += '<button type="button" class="connected" data-target="' + esc(row['key']) + '">Review achievement and supporting records together</button></section>'
    ...
    bootstrap={'review_id':session['review_id'],'proposal_sha256':session['proposal']['sha256'],
               'items':[{'key':r['key'],'fingerprint':r['fingerprint'],'decision':r.get('decision'), 'title':title(r), 'evidence_status':(r['after'] or {}).get('evidence_status') if isinstance(r['after'],dict) else None, 'source_refs':(r['after'] or {}).get('source_refs',[]) if isinstance(r['after'],dict) else [], 'review_status':r['review_status'], 'change':r['change']} for r in rows], 'groups':state.get('groups',[]),
