...
from evidence_rules import eligible  # noqa: E402
...
def metrics_block(atom):
    ...
    for metric in metrics:
        ...
        line = esc(text)
        if basis:
            label = 'measured' if isinstance(metric, dict) and metric.get('measured') is True else 'measurement information'
            line += f' <span class="basis">— {label}: {esc(basis)}</span>'
        else:
...
def atom_html(atom, employers, pack=None):
    ...
    if pack is not None:
        from pack_review import review_status
        state = review_status('evidence_atoms/' + atom['id'], pack)
        meta.append('<div><b>Your wording review:</b> ' + esc(state.replace('_', ' ')) + '</div>')
    parts.append('<div class="meta">' + "".join(meta) + "</div>")
    excerpts = ''.join('<p><b>' + esc(r.get('source_id', '')) + '</b> ' + esc(r.get('locator', '')) + '</p><blockquote>' + esc(r.get('excerpt') or 'No excerpt recorded') + '</blockquote>' for r in atom.get('source_refs', []))
    if excerpts:
        parts.append('<details><summary>Show original source excerpts</summary>' + excerpts + '</details>')
...
def build(pack, pack_path, digest):
    ...
    strengths = ''.join('<section class="meta"><h3>' + esc(item['interpretation']) + '</h3><p>Interpretation · ' + esc(profile_state(item, pack)) + '</p>' + show(item) + '</section>' for item in pack.get('strengths_profile', []))
    preferences = ''.join('<section class="meta"><h3>' + esc(item['text']) + '</h3><p>Future direction or preference, not an achievement.</p>' + show(item) + '</section>' for item in pack.get('positioning_preferences', []))
    review_help = '<section><h2>Your review</h2><p>Review whether this record reflects your contribution, not just whether fields are complete. A wording review is separate from evidence confidence and permission for external use.</p><p>Use the guided review workflow to compare a proposal, record corrections and accept changes. See <code>docs/pack-review.md</code>. This page is a read-only snapshot.</p></section>'
    body = [
        ...
        review_help,
        "<h2>Employment · My career timeline</h2>",
        ...
        *(["<h2>Education</h2>",
        ...
        "<h2>My strengths</h2>", strengths or "<p>No interpretations recorded yet.</p>",
        "<h2>What I want next</h2>", preferences or "<p>No preferences recorded yet.</p>",
        "<h2>Evidence atoms · My achievements</h2>",
        controls,
        "".join(atom_html(a, employers, pack) for a in atoms),
...
def main(argv):
    ...
    source.add_argument("--session", help="guided review of a staged proposal")
    source.add_argument("--pack", type=Path, default=None,
    ...
    if args.session:
        ...
        target = args.output or (ROOT / "outputs" / "career-review.html")
        target.parent.mkdir(parents=True, exist_ok=True)
        target.write_text(render_review(status(args.session)))
        print(target)
        return 0
