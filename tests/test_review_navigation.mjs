...
let socket, spawnError, reviewServer;
...
try {
    ...
    await check('Empty filters explain hidden pending work', `queue.length === 0 && !document.getElementById('empty-queue').hidden && document.getElementById('empty-queue').textContent.includes('80 records remain available')`);
    ...
    const workspace = path.join(temp, 'connected-workspace');
    execFileSync(process.env.PYTHON || 'python3', [path.join(root,'tests/build_review_navigation_fixture.py'), '--workspace', workspace]);
    reviewServer = spawn(process.env.PYTHON || 'python3', ['-B', path.join(root,'scripts/career_core.py'), 'review', 'open',
        '--session','reviews/pack-reviews/browser/session.json'], {env:{...process.env, CAREER_WORKSPACE:workspace}});
    const reviewURL = await new Promise((resolve,reject) => {
        let output = '';
        const timeout = setTimeout(() => reject(Error('Review server did not start')),10000);
        reviewServer.once('error',reject); reviewServer.once('exit',code => {clearTimeout(timeout);reject(Error('Review server exited: '+code));});
        reviewServer.stdout.on('data',chunk => {output += chunk; if (output.includes('\n')) {clearTimeout(timeout);resolve(output.split('\n')[0]);}});
    });
    assert.ok(reviewURL.startsWith('http://127.0.0.1:'));
    await cdp('Page.navigate',{url:reviewURL}); await delay(400);
    await check('Connected review starts with the role and five stable items', `data.grouped && queue.length === 15 && queue[0].dataset.group === 'employment' && cards.filter(c=>!c.hidden).length === 5`);
    await evaluate(`reviewer.value='Fictional Reviewer';queue[0].querySelector('.action').value='later';queue[1].querySelector('.action').value='accept';document.getElementById('next').onclick()`);
    await check('Pending dependencies preserve decisions and allow the next batch', `page === 1 && cards.every(c=>c.dataset.reviewed!=='accepted') && status.textContent.includes('Still to resolve')`);
    await evaluate(`document.getElementById('previous').click()`);
    await evaluate(`reviewer.value='Fictional Reviewer'; window.expectedNext=queue[5].dataset.key; for(const card of queue.slice(0,5)) card.querySelector('.action').value='accept'; document.getElementById('next').onclick()`);
    await check('Saving first five advances to the next item without skipping a batch', `queue[0].dataset.key === window.expectedNext && page === 0 && cards.filter(c=>c.dataset.reviewed==='accepted').length === 5`);
    assert.equal((await fs.readdir(path.join(workspace,'data/packs'))).length,1);
    await evaluate(`for(const card of queue.slice(0,5)) card.querySelector('.action').value='accept'; document.getElementById('next').onclick()`);
    await check('Second batch saves without reapproving the role', `cards.filter(c=>c.dataset.reviewed==='accepted').length === 10 && queue.length === 5`);
    assert.equal((await fs.readdir(path.join(workspace,'data/packs'))).length,2);
    await evaluate(`document.getElementById('save-pack').onclick()`);
    assert.equal((await fs.readdir(path.join(workspace,'data/packs'))).length,2);
    results.push('Repeated browser save creates no extra pack');
    await check('Connected narrow page fits and offers the accepted reading view', `document.documentElement.scrollWidth <= innerWidth && !document.getElementById('reading-link').hidden && document.querySelector('.review-navigation').getBoundingClientRect().height < innerHeight/2`);
    const connectedScreenshot=await cdp('Page.captureScreenshot',{format:'png'});
    if(process.env.CAREER_REVIEW_SCREENSHOT) await fs.writeFile(process.env.CAREER_REVIEW_SCREENSHOT.replace('.png','-connected.png'),Buffer.from(connectedScreenshot.data,'base64'));
    await evaluate(`window.editedKey=queue[0].dataset.key; const editor=queue[0].querySelector('[data-field="star.action"]');editor.value='I co-designed this with the operations team.';editor.dispatchEvent(new Event('input', {bubbles:true}));`);
    await cdp('Page.reload');await delay(400);
    await check('Unsent wording edits survive a page reload', `queue[0].querySelector('[data-field="star.action"]').value === 'I co-designed this with the operations team.'`);
    await evaluate(`const other=queue[1].querySelector('[data-field="star.action"]');other.value='I mentored a colleague through the review.';other.dispatchEvent(new Event('input',{bubbles:true}));queue[0].querySelector('.correct-wording').click()`);await delay(700);
    await check('Browser correction opens revised wording without accepting it', `data.review_id.startsWith('correction-') && cards.some(c=>c.textContent.includes('I co-designed this with the operations team.') && c.dataset.reviewed!=='accepted' && !c.querySelector('.action').value)`);
    assert.equal((await fs.readdir(path.join(workspace,'data/packs'))).length,2);
    await check('Preview retains edits on other cards', `cards.some(c=>c.querySelector('[data-field="star.action"]')?.value==='I mentored a colleague through the review.' && !c.querySelector('.action').value)`);
    await evaluate(`const card=cards.find(c=>c.querySelector('[data-field="star.action"]')?.value==='I co-designed this with the operations team.');card.querySelector('[data-field="star.action"]').value='I tested the team design with three colleagues.';card.querySelector('.action').value='accept';document.getElementById('save-pack').click()`);await delay(700);
    await check('Ordinary Save previews dirty wording and requires fresh confirmation', `cards.some(c=>c.querySelector('[data-field="star.action"]')?.value==='I tested the team design with three colleagues.' && !c.querySelector('.action').value) && status.textContent.includes('ready to review')`);
    assert.equal((await fs.readdir(path.join(workspace,'data/packs'))).length,2);
    await evaluate(`const card=cards.find(c=>c.querySelector('[data-field="star.action"]')?.value==='I tested the team design with three colleagues.');card.querySelector('.action').value='accept';document.getElementById('save-pack').onclick()`);
    await check('Fresh confirmation saves the exact revised wording', `cards.some(c=>c.querySelector('[data-field="star.action"]')?.value==='I tested the team design with three colleagues.' && c.dataset.reviewed==='accepted')`);
    assert.equal((await fs.readdir(path.join(workspace,'data/packs'))).length,3);
    assert.equal(errors.length,0,JSON.stringify(errors));
    console.log(results.map(name => 'PASS ' + name).join('\n'));
    console.log(`${results.length} browser navigation checks passed`);
} finally {
    if (socket) socket.close(); if(reviewServer) reviewServer.kill('SIGTERM'); child.kill('SIGTERM'); await delay(500);
