...
The page starts with added/changed/removed counts and shows five items at a time.
...
python3 scripts/career_core.py review publish --session reviews/pack-reviews/onboarding-v1/session.json --output data/packs/career-reviewed-v1.json
```

Here `publish` means saving a **local accepted pack version**, not sending anything
externally. It uses only explicit accepted items and privacy restrictions. It
